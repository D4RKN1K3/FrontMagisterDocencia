...
const DocumentCRUD = ({ name, urls, title, subtitle }) => {
  ...
  const options = [
    { label: `Identificador del ${itemName}`, value: 'documentID' },
    { label: `Categoría del ${itemName}`, value: 'category' },
    { label: `Formato del ${itemName}`, value: 'format.name' },
  ];
  ...
  try {
    ...
    if (access_token) {
      ...
      if (selectedItems.length !== 0) {
        const idsToDelete = selectedItems.map(item => item.documentID);
...
const handleCheckboxChange = (event, item) => {
  if (event.target.checked) {
    ...
    setSelectedItems((prevSelectedItems) =>
      prevSelectedItems.filter((selectedItem) => selectedItem.documentID !== item.documentID)
    );
  }
};
...
const [sortProperty, setSortProperty] = useState('documentID');
...
return (
  <div>
    ...
    <ModalCRUD isOpen={ModalOpen}>
      ...
      >
        <DynamicSelect selectId='documentCategory' label="Seleccione una Categoría" options={validCategory} value={newItem.category} onChange={(e) => setNewItem({ ...newItem, category: e.target.value })} />
        {(!updateId) && (
          ...
          {getCurrentPageItems().map((item) => (
            <div key={item.documentID} className="flex flex-col justify-center text-center relative">
              <div className="absolute top-0 left-0">
                <Checkbox
                  id={`deleteInput-${item.documentID}`}
                  name={`deleteInput-${item.documentID}`}
                  checked={selectedItems.some((selectedItem) => selectedItem.documentID === item.documentID)}
              ...
              <h4 className="text-xl font-semibold">{item.documentID}</h4>
              <p className="text-sm text-gray-700 mb-2">{item.category}</p>

              <div className="flex flex-col items-center justify-center gap-2">
                <div className="w-full flex-1 sm:w-52">
                  <a href={item.archive} target="_blank" rel="noopener noreferrer">
                    <CustomButton
                      type="button"
                      color='orange'
                      padding_x='4'
