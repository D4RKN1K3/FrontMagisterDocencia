...
const SpecializationHasSemesterSection = ({ items, semester, selectedItems, handleDeleteSelected, handleCheckboxChange, handleEdit, children, updateId, itemName, handleSubmit, closeModal }) => {
        ...
        <section className="bg-white text-orange-400">
            <div className="container mx-auto flex flex-col">
                <div className="divide-y divide-orange-500">
                    ...
                    {items.map((item) => (
                        <div key={item.specializationHasSemesterID}
                            className={`${(updateId && updateId === item.specializationHasSemesterID) || (selectedItems.length !== 0 && selectedItems[0].specializationHasSemesterID === item.specializationHasSemesterID) ? ' mb-1 sm:mb-2' : 'mb-12 sm:mb-2'} relative grid justify-center grid-cols-5 py-2 sm:py-4 mx-auto sm:space-y-4 sm:space-y-0`}>
                            {(item.evaluationStatusID === 1) && (
                                <>
                                    {(!updateId && updateId !== item.specializationHasSemesterID) && (
                                        <div className="absolute top-6 left-6">
                                            <Checkbox
                                                id={`deleteInput-${item.specializationHasSemesterID}`}
                                                name={`deleteInput-${item.specializationHasSemesterID}`}
                                                checked={selectedItems.some((selectedItem) => selectedItem.specializationHasSemesterID === item.specializationHasSemesterID)}
                            ...
                            <div className="flex flex-col justify-center max-w-3xl text-center col-span-full sm:col-span-4 sm:text-left">
                                <div className="flex items-center justify-between flex-col sm:flex-row">
                                    ...
                                    <div className={`text-sm sm:text-md px-2 py-1 mt-1 sm:mt-0 font-bold rounded ${getStatusClass(item.statusID)} text-white`}>
                                        {item.statusID
                                            ? `${item.statusName}`
                                ...
                                {(selectedItems.length !== 0 && selectedItems[0].specializationHasSemesterID === item.specializationHasSemesterID) && (
                                ...
                                <div className={`${(updateId && updateId === item.specializationHasSemesterID) || (selectedItems.length !== 0 && selectedItems[0].specializationHasSemesterID === item.specializationHasSemesterID) ? 'hidden' : 'flex'} flex flex-col sm:flex-row gap-1 sm:gap-2 h-10`}>
                                    {(item.evaluationStatusID === 1) && (
...
export default SpecializationHasSemesterSection;
